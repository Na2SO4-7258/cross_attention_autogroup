...
def main():
    ...
    ensure_fivek(cfg.data_dir,cfg.download_url);records=discover_records(cfg.data_dir);splits=make_splits(records,cfg.output_dir,cfg.seed,(cfg.train_ratio,cfg.val_ratio));train=FiveKDataset(records,splits["train"],cfg.image_size);val=FiveKDataset(records,splits["val"],cfg.image_size);test=FiveKDataset(records,splits["test"],cfg.image_size)
    ...
    model=LatentRetouchModel(cfg.attention_dim,cfg.edit_dim,cfg.cross_attention_heads,cfg.attention_spatial_size,cfg.encoder_base_dim,cfg.encoder_mid_dim,cfg.enhancer_base_dim,cfg.enhancer_mid_dim);trainer=Trainer(model,cfg,train,val);start=1;best=-float("inf")
    if args.resume:
