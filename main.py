...
def main():
    ...
    if args.resume:
        state=torch.load(args.resume,map_location=cfg.device);model.load_state_dict(state["model_state"]);trainer.optimizer.load_state_dict(state["optimizer_state"]);loaded_refs=state.get("references",{});trainer.references=loaded_refs if all(key in loaded_refs for key in ("train","val","test")) else {"train":loaded_refs,"val":{},"test":{}};trainer.temperature=state.get("temperature",cfg.temperature);start=state["epoch"]+1;best=state.get("best_psnr",best)
    else:
    ...
    for epoch in range(start,cfg.num_epochs+1):
        ...
        if args.mode=="dynamic_attention":
            row={"epoch":epoch,"temperature":trainer.temperature,"train_grouping":trainer.update_grouping(train,"train",epoch)}
