...
def _save_visualization(path,original,reference,output,target,psnr,ssim):
    """Save original/reference/output/target with per-image PSNR and SSIM."""
...
def save_visualizations(model,dataset,trainer,mode,name,max_images=None):
    """Render examples; ``max_images=None`` means the full dataset."""
    ...
    for index in range(limit):
        ...
        out,_,_=model(o,ro,re,trainer.temperature);psnr=psnr_per_image(out,t).item();ssim=ssim_per_image(out,t).item()
